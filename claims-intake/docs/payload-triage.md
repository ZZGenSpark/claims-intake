...
400 list was JSON, missing field, wrong type, or extra field.
Open whether that is 400 or V-5.

**Decision.** Reject. `UNINTERPRETABLE_REQUEST`, 400.

**Authority.** Section 2.2: `claim_type` is one of the values
in 2.3. The vocabulary is the type of the field. A value
outside it cannot be interpreted, so no rule runs.

**Rejected alternative.** V-5 `TYPE_NOT_COVERED` 422. V-5
compares a vocabulary value to the product's permitted
subset. `flood` is not in the vocabulary.

**Contract amended.** 2.3, 2.4, 4.2, and section 6: a
`claim_type` outside 2.3 is `UNINTERPRETABLE_REQUEST`.
